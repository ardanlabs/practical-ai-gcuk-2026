...
func buildEntities(ctx context.Context, g *age.Graph, krnChat *kronk.Kronk, chunks []chunk) error {
	// Resumable: a chunk is logged as extracted only once its nodes and edges are
	// in the graph, so a ctrl-c costs the chunk in flight and nothing else.
	done, err := storedIDs(ctx, g.DB(), `SELECT chunk_id FROM extracted`)
	if err != nil {
	...
	if len(todo) == 0 {
		fmt.Printf("- %d chunks already extracted\n", len(done))
		return nil
	}

	fmt.Printf("\nExtracting entities from %d of %d chunks, one model call per chunk. This takes a while.\n",
		len(todo), len(chunks))
	fmt.Print("\033[s")

	const logExtracted = `
	INSERT INTO extracted (chunk_id, entities)
	VALUES ($1, $2)
	ON CONFLICT (chunk_id) DO NOTHING`

	stmt, err := g.DB().PreparexContext(ctx, logExtracted)
	if err != nil {
	...
	for i, c := range todo {
		...
		fmt.Printf("Extracting: %d of %d (entities %d, relationships %d, unparsable %d)",
			i+1, len(todo), entityCount, relCount, failed)
		...
		// ---------------------------------------------------------------------
		// Logged last, so an interrupted chunk is extracted again next run.

		if _, err := stmt.ExecContext(ctx, c.id, chunkEntities); err != nil {
			return fmt.Errorf("log extracted %d: %w", c.id, err)
		}
	}
...
//
// A hub subject, one a twentieth of the book mentions, joins chunks that have
// nothing to do with each other, so it is not a hop.
func entityHop(ctx context.Context, g *age.Graph, ids []int, perEntity int, hubs map[string]bool) (map[int]string, error) {
	q := fmt.Sprintf(`
