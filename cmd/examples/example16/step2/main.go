// This example shows you a RAG application that retrieves through a graph. It
// uses Apache AGE for the graph, pgvector for embeddings, and the Kronk SDK.
//
// Step1 measured what plain similarity misses. Step2 buys it back with one hop:
//
//   - every chunk is a node, consecutive chunks are joined by a NEXT edge
//   - each chapter is a Section node over the chunks it contains
//   - retrieval runs the vector search, then walks NEXT out of every hit
//
// Nothing here calls a model. These edges were already in the book.
//
// # Running the example:
//
//	$ make compose-up
//	$ make example16-step2
...
const (
	...
	// contextWindow is the same as step1 because the NEXT hop adds at most a
	// chunk per hit. Step3 doubles it, the first cost of retrieving through a
	// graph.
	contextWindow = 16384
...
func run() error {
	...
	if err := loadData(context.Background(), g, krnEmbed, chunksFile); err != nil {
...
// graphSearch performs the retrieval and prints what came back. Anything marked
// NEXT is a chunk the vector search did not rank high enough on its own.
func graphSearch(ctx context.Context, krnEmbed *kronk.Kronk, g *age.Graph, messages []model.D) ([]document, error) {
	...
	for _, doc := range docs {
		fmt.Printf("[%-6s sim=%.4f id=%-3d %s] %s\n",
			doc.Hop, doc.Similarity, doc.ID, doc.Chapter, preview(doc.Text, 80))
	}
